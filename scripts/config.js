...
Hooks.on("getSceneControlButtons", (controlButtons) => {
  controlButtons.tiles.tools.tileSort = {
    name: "tileSort",
    title: "Tile Sort",
    icon: "fas fa-sort-numeric-down-alt",
    button: true,
    onChange: () => {
      const tileSort = Object.values(ui.windows).find(
        (w) => w.id === "tile-sort"
      );
...
Hooks.on("renderTileSort", (app, html) => {
  ...
  app.position.top = 2;
  app.position.left =
    $(window).width() - app.element.width() - $("#sidebar").width() - 15;
